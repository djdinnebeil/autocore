...
Person-name journal aliases live in `journal_choices.ini` under the configured journal data directory (default `dist/components/journal/`). `journal_config.exe` writes a missing live file from [`defaults/components/journal/journal_choices.ini`](defaults/components/journal/journal_choices.ini). An existing file is never overwritten. Unused aliases are not bound; `keymap.map` is the filter.
...
│  ├─ components.list   Sample component registry
│  ├─ keymap.map        Sample key map
│  ├─ components/journal/  Sample journal_choices.ini
│  └─ server/           Shipped document root (copied to dist/components/server on build)
