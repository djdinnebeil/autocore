...
int first_time() {
    ...
    if (!write_values(*values)) {
        shutdown_config.log_and_print(
            "Failed to write {}.",
            ini_path().string()
        );
        return 1;
    }
    shutdown_config.log_and_print("Wrote {}", ini_path().string());
    return 0;
}
...
int main() {
    shutdown_config.connect_to_logger();
    shutdown_config.log_and_log("shutdown_config.exe started");
