 ...
 * \brief Generic host: open `components.list`, hello, invoke, shutdown.
 */
module;
...
namespace {
    ...
    const ac::config::components_list::ParseResult& loaded_catalog() {
        if (!catalog_loaded) {
            catalog = ac::config::components_list::load(
    ...
    ShutdownSettings load_shutdown_settings() {
        ...
        const auto document = ac::ini::read(
            ac::paths::config_directory() / "shutdown.ini"
        );
        if (!document) {
            return settings;
        }

        if (const auto prompt = document->find(
                "shutdown", "delayed_shutdown_prompt"
            ); prompt && *prompt == "console") {
            settings.prompt = DelayedShutdownPrompt::console;
        }

        if (const auto timeout = document->find(
    ...
    std::optional<Launch> try_launch(const std::string& name) {
        ...
        const auto executable =
            ac::paths::executable_directory() / protocol::executable_name(name);
...
void ac::main::components::shutdown(const bool allow_recovery_prompt) {
    ...
    auto lingering = lingering_children();
    if (!lingering.empty()) {
        log_lingering(lingering, shutdown_started);
...
ac::main::components::Session ac::main::components::initialize() {
    ...
    if (!parsed.ok) {
        auto_core.log_and_print("{}", parsed.error);
        return session;
    }

    for (const auto& name : parsed.invalid_names) {
        auto_core.log_and_print(
            "Invalid component name in components.list is ignored: {}",
            name
        );
    }
