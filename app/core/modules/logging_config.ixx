 ...
 *
 * Settings are loaded from `config/logger.ini` under the executable directory.
...
export namespace ac::logging::config {
    ...
    /**
     * \brief Returns whether component logging is enabled.
     *
     * Reads `[logger] enabled` from `logger.ini`. The default is `true`.
     * Only lowercase `true` and `false` are accepted; any other value, or a
     * missing key, keeps that default. This flag does not control console
     * output; see `write_to_console()`.
     */
     ...
     *
     * Reads `[logger] write_to_console`. The default is `false`. Only
     * lowercase `true` and `false` are accepted; any other value, or a
     * missing key, keeps that default.
     */
