...
export module auto_core.core.clipboard;
...
export namespace ac::clipboard {
     ...
     *
     * Paste uses `ac::keyboard::send_key_combination`. A send failure maps
     * to `Error::paste_failed`. Input processing is asynchronous. Callers
     * that replace the clipboard immediately afterward must allow the
     * target time to process the paste.
     */
    [[nodiscard]]
    AC_API std::expected<void, Error>
