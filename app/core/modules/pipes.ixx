...
export module auto_core.core.pipes;
...
export namespace ac::pipes {
     ...
     *
     * The dispatcher is neither copyable nor movable. `request_stop()` does
     * not cancel a `ReadFile` that is already in progress.
     */
    class CommandDispatcher {
    public:
        /** \brief Constructs a dispatcher with no registered commands. */
         ...
         *
         * This call blocks on pipe I/O. An unregistered command fails with
         * `ERROR_INVALID_DATA`. Exceptions thrown by a registered action
         * propagate to the caller. `request_stop()` does not cancel a
         * `ReadFile` that is already in progress.
         *
     ...
     *
     * This does not wait for a client. Connection completes when
     * `connect_to_pipe_server` succeeds; the first read or write then
     * proceeds.
     *
