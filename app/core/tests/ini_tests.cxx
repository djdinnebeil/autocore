...
TEST_CASE("INI settings are returned in file order", "[ini][unit]") {
    const auto document = ac::ini::parse(
        "[devices]\n"
        "desktop =\n"
        "laptop = abc\n"
    ...
    const auto devices = document.settings("devices");
    REQUIRE(devices.size() == 3);
    CHECK(devices[0].first == "desktop");
    CHECK(devices[0].second.empty());
    CHECK(devices[1].first == "laptop");
    CHECK(devices[1].second == "abc");
    CHECK(devices[2].first == "mobile");
    CHECK(document.settings("missing").empty());
}
