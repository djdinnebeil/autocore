...
constexpr std::string_view component_name = "simple_test_copy";
constexpr std::string_view executable_name = "simple_test_copy_ac.exe";

ac::Component& simple_test_copy_component() {
    static ac::Component component {component_name};
    return component;
}

command_registry::Registry create_simple_test_copy_command_registry() {
    command_registry::Registry registry;
    registry.add("print_simple_test_copy", [] {
        simple_test_copy_component().log_and_print(
            "this is print_simple_test_copy() from within simple_test_copy_ac.exe"
        );
    });
    return registry;
}
...
int main(int argument_count, char* arguments[]) {
    auto registry = create_simple_test_copy_command_registry();
    ...
    auto& component = simple_test_copy_component();
    component.connect_to_logger();

    const auto pipe_name = ac::protocol::component::pipe_name(component_name);
    component.log_and_print("simple_test_copy_ac.exe starting");
    ...
    if (!connection) {
        ...
        component.log_and_print(
            "Failed to connect to simple_test_copy pipe. Error: {}",
    ...
    dispatcher.set_command(
        ...
        [&pipe, &registry, &dispatcher, &protocol_failed, &component] {
            ...
            if (!expression) {
                component.log_and_print(
                    "Failed to read simple_test_copy command. Error: {}",
            ...
            if (!action) {
                component.log_and_print(
                    "Unknown simple_test_copy command: {}",
                    *expression
                );
    ...
    if (const auto hello = ac::pipes::send_string(
        ...
        component.log_and_print(
            "Failed to send simple_test_copy hello. Error: {}",
    ...
    if (const auto result = dispatcher.process(pipe); !result) {
        component.log_and_print(
            "simple_test_copy pipe failed. Error: {}",
