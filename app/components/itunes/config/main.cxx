...
int main(int argc, char* argv[]) {
    ...
    itunes_config.connect_to_logger();
    itunes_config.log_and_log("itunes_config.exe started");
    ...
    if (initialize) {
        ...
        itunes_config.log_and_print("Wrote {}", path.string());
        return ac::config::components_request::run_component_update("itunes");
    }
    ...
    if (!std::getline(std::cin, auto_start_input)) {
        itunes_config.log_and_print("Failed to read auto_start.");
        return 1;
    }
    ...
    if (!std::getline(std::cin, tab_input)) {
        itunes_config.log_and_print("Failed to read tab_end.");
        return 1;
    }
