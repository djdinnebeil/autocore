...
namespace {
    ...
    bool write_keymap_manifest(const ToolConfiguration& configuration) {
        ...
        const auto destination =
            executable_directory() / L"keymap" / L"components" / L"taskbar.keymap_commands.txt";
        std::error_code error;
    ...
    bool ensure_taskbar_ini() {
        ...
        const auto directory = trim(std::wstring_view {input});
        const auto utf8 = to_utf8(directory);
        if (!utf8) {
            std::cerr << "Failed to encode the taskbar data directory.\n";
            return false;
        }
        return ac::config::write_taskbar_ini_if_missing(
            utf8->empty() ? "taskbar" : *utf8
        );
    }
