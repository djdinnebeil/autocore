...
void Spotify::get_current_song() {
    spotify_component.logg("get_current_song()");
    if (!refresh_tokens()) {
        return;
    }
    ...
    if (response.status_code != 200) {
        if (response.status_code == 204) {
            spotify_component.logg("{} status code", response.status_code);
            is_playing = false;
        }
        else {
            spotify_component.logg_and_print("{} status code", response.status_code);
        }
        return;
    }
    ...
    if (!song_details["item"].contains("name")) {
        spotify_component.logg_and_logg("djai++ is talking");
        last_status_code = 15;
        return;
    }
    ...
    spotify_component.loggnl_and_loggnl("now playing: ");
    spotify_component.logg_and_print(current_song);
...
std::string Spotify::get_user_queue() {
    try {
        ...
        if (response.status_code != 200) {
            spotify_component.logg_and_print("Failed to retrieve queue");
            return "";
        }
        ...
        spotify_component.logg_and_print("An exception occurred in get_user_queue");
        return "";
    }
}
/**
...
bool Spotify::download_album_cover() {
    ...
    if (response.status_code != 200) {
        spotify_component.logg_and_print("Failed to retrieve song");
        return "";
    }
    auto song_details = parse(response.text);
