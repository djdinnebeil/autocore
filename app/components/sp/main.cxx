...
void set_command_map() {
    ...
    command_map[5] = {update_sp_component};
...
int main() {
    ...
    HANDLE ac_sp_pipe = ac::pipes::connect_to_pipe_server(pipe_name, sp_component);
    if (ac_sp_pipe != NULL) {
        sp_component.logg_and_logg("connected to pipe '{}' server", pipe_name);
        ac::pipes::process_pipe_commands(ac_sp_pipe, sp_component);
    }
    else {
        sp_component.logg_and_print("Failed to connect to pipe server.");
    }
